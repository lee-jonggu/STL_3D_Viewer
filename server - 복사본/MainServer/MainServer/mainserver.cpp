...
void MainServer::readClient()
{
    ...
    if (byteReceived == 0) {                                                                             // 파일전송 시작할 경우
        ...
        qDebug() << "????" <<totalSize<<byteReceived<<filename<<status;
...
void MainServer::goOnSend(qint64 numBytes)
{
    ...
    receivedSocket->write(outBlock);                                                           // 파싱한 파일을 소켓으로 보냄
    receivedSocket->flush();
}
...
void MainServer::sendFile()
{
    ...
    if(filename.length()) {
        ...
        qDebug("4");
        while(receivedSocket->waitForBytesWritten());
        qDebug("5");
    }
