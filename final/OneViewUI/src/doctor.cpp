...
//    connect(clientSocket, SIGNAL(disconnected( )), SLOT(disconnect( )));                        // 연결 종료시 disconnect 슬롯
    ...
    for (int year = QDate::currentDate().year(); year >= 1900; --year) {
        ui->patientBirthYcomboBox->addItem(QString::number(year));
    }

    for (int month = 1; month <= 12; ++month) {
        ui->patienBirthMcomboBox->addItem(QDate::longMonthName(month));
    }

    for (int day = 1; day <= 31; ++day) {
        ui->patienBirthDcomboBox->addItem(QString::number(day));
    }
    ...
    case Send_Info:
        if (fromWho == server)
            for (int i = 0; i < patientNum; i++)
            {
                ...
                patientGender = patient.split("/c/")[3];
                patientFirstVisitDate = patient.split("/c/")[4];
                patientLastVisitDate = patient.split("/c/")[5];
                patientMobile = patient.split("/c/")[6];
                patientPhone = patient.split("/c/")[7];
                patientEmail = patient.split("/c/")[8];
                patientEmailDomain = patient.split("/c/")[9];
                patientAddress = patient.split("/c/")[10];
                patientDetailAddress = patientAddress.split("/addr/")[1];
                patient_stl_path = patient.split("/c/")[11];
...
void Doctor::sendData()
{
    ...
    patientBirthY = ui->patientBirthYcomboBox->currentText();
    patienBirthM = ui->patienBirthMcomboBox->currentText();
    patienBirthD = ui->patienBirthDcomboBox->currentText();
    ...
    if (ui->patientEmaillineEdit2 == nullptr)
    {
...
void Doctor::loadSTLFile()
{
    ...
    ui->patientDataupperopenGLWidget->interactor()->SetRenderWindow(mWindow);
